...
void __stdcall printDigit(LPSTR mes, DWORD digit){
	LPSTR Out = new CHAR(50);
	wsprintf(Out, "%i\n", digit);
...
{ 
	switch (State){
		case D3DTS_PROJECTION:{
			...
			//pMatrixNew->m[0][0] /=  ((float)w/(float)h) / (4.0/3.0) ;
			if (gamerender)
				pMatrixNew->m[1][1] *= 1.3;//vertical FoV
				//pMatrixNew->m[0][0] /= 1.3;// horizontal Fov
			return (m_pIDirect3DDevice8->SetTransform(State, pMatrixNew));
		};
		default:return (m_pIDirect3DDevice8->SetTransform(State, pMatrix));
	}
}
...
{
	D3DVIEWPORT8* pViewportNew = new D3DVIEWPORT8(*pViewport);
	//some shitty constants to determine region where actual game draws
	gamerender = pViewport->Y >= ((double)h*0.02) && pViewport->Y <= ((double)h*0.05)
		&& pViewport->Height >= h*0.73 && pViewport->Height <= h*0.76;
	if (w == NULL || pViewport->Width > w)
		w = pViewport->Width;
	if (h == NULL || pViewport->Height > h)
		h = pViewport->Height;
	return (m_pIDirect3DDevice8->SetViewport(pViewportNew));
}
